  ...
  return (
    <>
      ...
      {data.type === "HTTP Request" && (
        ...
        >
          <DialogTitle 
            sx={{ borderBottom: '1px solid #444', padding: '16px',
          ...
          <DialogContent sx={{ padding: '24px' }}>
            <FormControl fullWidth margin="normal">
              ...
              <Select labelId="method-label" name="method" value={formData.method} onChange={handleFormChange}
            ...
            <TextField fullWidth margin="normal" label="URL" name="url" value={formData.url} onChange={handleFormChange}
              sx={{ marginTop: '24px', '& .MuiOutlinedInput-root': { color: '#fff', '& fieldset': {  borderColor: '#444' },
                  '&:hover fieldset': { borderColor: '#666' },
                  '&.Mui-focused fieldset': { borderColor: '#ff6d5a' }
                },
                '& .MuiInputLabel-root': { color: '#888','&.Mui-focused': {color: '#ff6d5a' } }
              }}
            />
            <FormControl fullWidth margin="normal" sx={{ marginTop: '24px' }}>
              ...
              <Select labelId="auth-label" name="authentication" value={formData.authentication} onChange={handleFormChange}
                sx={{ color: '#fff',
            ...
            <Button  variant="contained"  onClick={handleClose}
              sx={{  mt: 4, backgroundColor: '#ff6d5a',
      ...
      {data.type === "Click Trigger" && (
        <Dialog  open={isDialogOpen}  onClose={handleClose} maxWidth={false}
          PaperProps={{
            style: { backgroundColor: '#2a2a2a', color: '#fff', width: '40%', borderRadius: '8px', padding: '16px' } }} >
          <DialogTitle 
            sx={{  borderBottom: '1px solid #444', padding: '16px',
          ...
          <DialogContent sx={{ padding: '24px' }}>
            <Box sx={{ height: '400px', border: '1px solid #444', borderRadius: '4px' }}>
              <MonacoEditor height="400px" defaultLanguage="json" theme="vs-dark" value={scheduleOutput} onChange={handleScheduleOutputChange}
                options={{ minimap: { enabled: false }, scrollBeyondLastLine: false, fontSize: 14, lineNumbers: 'on', roundedSelection: false,
                scrollBars: 'vertical', automaticLayout: true }}/>
            </Box>
      ...
      {data.type === "Code" && (
        <Dialog  open={isDialogOpen}  onClose={handleClose} maxWidth={false}
          PaperProps={{
            style: { backgroundColor: '#2a2a2a', color: '#fff', width: '40%', borderRadius: '8px', padding: '16px' } }}>
          <DialogTitle 
            sx={{  borderBottom: '1px solid #444', padding: '16px', '& .MuiTypography-root': { fontSize: '1.5rem', fontWeight: 600 } }}>
            Code
          </DialogTitle>
          <DialogContent sx={{ padding: '24px' }}>
            <FormControl fullWidth margin="normal" sx={{ mt: 3 }}>
              ...
              <Select labelId="language-label" value="Code"
                sx={{ color: '#fff', '& .MuiOutlinedInput-notchedOutline': { borderColor: '#444'},
                  '&:hover .MuiOutlinedInput-notchedOutline': { borderColor: '#666' },
                  '&.Mui-focused .MuiOutlinedInput-notchedOutline': {borderColor: '#ff6d5a' }
                }}>
            ...
            <Box sx={{ height: '400px', border: '1px solid #444', borderRadius: '4px', mt: 3 }}>
              <MonacoEditor height="400px" defaultLanguage="javascript" theme="vs-dark"  value={jsCode} onChange={handleCodeChange}
                options={{ minimap: { enabled: false }, scrollBeyondLastLine: false, fontSize: 14, lineNumbers: 'on', roundedSelection: false,
                  scrollBars: 'vertical', automaticLayout: true }} />
            </Box>
            <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 2, mt: 3 }}>
      ...
      {isCredentialDialogOpen && (
        <Dialog  open={isCredentialDialogOpen}  onClose={handleCredentialDialogClose} maxWidth={false}
          PaperProps={{
            style: { backgroundColor: '#2a2a2a', color: '#fff', width: '40%', borderRadius: '8px', padding: '16px' }}}>
          <DialogTitle 
            sx={{  display: 'flex', alignItems: 'center', gap: 1, borderBottom: '1px solid #444', padding: '16px',
              '& .MuiTypography-root': { fontSize: '1.5rem', fontWeight: 600} }} >
            <IconButton  onClick={handleCredentialDialogClose} sx={{ color: '#fff' }} >
              <ArrowBack />
            </IconButton>
            <Typography variant="subtitle1" 
              sx={{  color: '#ffffff', marginLeft: '8px', fontSize: '1rem' }}>
              Chat Model
            </Typography>
          </DialogTitle>
          <DialogContent sx={{ padding: '24px' }}>
            <Box sx={{ display: 'flex', gap: 2 }}>
              <Box sx={{ flex: 1 }}>
                <Button
                  sx={{ color: '#fff', backgroundColor: '#3a3a3a', padding: '12px 24px', width: '100%', justifyContent: 'flex-start',
          ...
          <DialogActions sx={{ padding: '16px', borderTop: '1px solid #444' }}>
            <Button variant="contained" onClick={() => {
              ...
              sx={{backgroundColor: '#ff6d5a', color: '#fff','&:hover': { backgroundColor: '#ff8d7a' }}} >
              Save
            </Button>
          </DialogActions>
        </Dialog>
      ...
      {isChatDialogOpen && (
        <Dialog  open={isChatDialogOpen}  onClose={() => setIsChatDialogOpen(false)} maxWidth={false}
          PaperProps={{
            style: { backgroundColor: '#2a2a2a', color: '#fff', width: '50%', borderRadius: '8px', padding: '16px' }}}>
          <DialogTitle 
            sx={{  borderBottom: '1px solid #444', padding: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }} >
            <Typography variant="h6">Chat Interface</Typography>
            <IconButton  onClick={() => setIsChatDialogOpen(false)}
              sx={{ color: '#fff' }} >
              <Close />
            </IconButton>
          </DialogTitle>
          <DialogContent sx={{ padding: '24px' }}>
            <Box 
              ...
              {/* Messages Area */}
              <Box 
                sx={{  flex: 1, overflowY: 'auto', p: 2, display: 'flex', flexDirection: 'column', gap: 1, backgroundColor: '#1a1a1a', borderRadius: '8px',
                       marginBottom: 2 }} >
                {messages.map((msg, index) => (
                  <Box  key={index}
                    sx={{ alignSelf: msg.role === "user" ? 'flex-end' : 'flex-start', backgroundColor: msg.role === "user" ? '#ff6d5a' : '#3a3a3a',
                      color: '#fff', p: 2, borderRadius: '8px', maxWidth: '80%'}}>
              ...
              <Box sx={{ display: 'flex', gap: 1 }}>
                <TextField fullWidth value={input} onChange={(e) => setInput(e.target.value)} placeholder="Type your message..."
                  onKeyPress={(e) => e.key === 'Enter' && handleSendMessage()}
                  sx={{'& .MuiOutlinedInput-root': {color: '#fff',backgroundColor: '#1a1a1a',
