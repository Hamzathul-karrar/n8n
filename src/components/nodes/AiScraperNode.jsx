...
export default function AiScraperNode({ data, id }) {
  ...
  const handleExecute = async (inputData) => {
    ...
    try {
      ...
      // First check if input is coming from ClickTriggerNode
      ...
      // Then check if connected to ChatTriggerNode
      else {
        const askQuestion = data.getChatCallback?.('ChatTrigger');
        if (askQuestion) {
          // Use chat trigger for input
          ...
          // If neither ClickTrigger nor ChatTrigger is connected, use dialog
